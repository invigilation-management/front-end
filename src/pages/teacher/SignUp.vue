            ...
            <el-card class="detail">
                <div>
                    ...
                    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="184px" class="demo-ruleForm" label-position="left">
                        ...
                        <el-form-item label="监考批次" prop="batchname">
...
import {addSignUp} from '../../api/teacher'
...
export default {
  ...
  methods: {
    ...
    handleSignUp () {
      this.$confirm('是否确认提交报名?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.signUpInfo.facultyFacultyId = this.ruleForm.ID
        this.signUpInfo.trueFacultyId = this.ruleForm.ID
        this.signUpInfo.batchBatchName = this.ruleForm.batchname
        this.signUpInfo.trueFacultyName = this.ruleForm.name
        this.signUpInfo.college = this.ruleForm.unit
        if (this.ruleForm.gender === '男') {
          this.signUpInfo.gender = 1
        } else {
          this.signUpInfo.gender = 0
        }
        this.signUpInfo.eduQualification = this.ruleForm.education
        this.signUpInfo.tele = this.ruleForm.tele
        this.signUpInfo.teleBackup = this.ruleForm.tele2
        this.signUpInfo.birthday = this.ruleForm.birth
        this.signUpInfo.avatar = this.ruleForm.imageUrl
        this.signUpInfo.targetCampus = this.ruleForm.school[0]
        console.info(this.signUpInfo)
        addSignUp(this.signUpInfo).then(res => {
          if (res.data === true) {
            this.$message({
              message: '提交成功',
              type: 'success'
